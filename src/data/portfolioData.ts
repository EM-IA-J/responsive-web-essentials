export const portfolioItems = [
  {
    id: 1,
    title: "Brillabruja",
    category: "Branding",
    tags: ["Ilustración", "Branding", "Identidad"],
    imageUrl: "brillabruja.png",
    link: "#",
    client: "Brillabruja",
    description: "Diseño creativo e innovador para una marca mágica con personalidad única y atractiva.",
    svgFiles: ["Brillabruja_1 (1).svg", "Brillabruja_2.svg", "Brillabruja_3.svg"],
    additionalImages: [],
    videos: []
  },
  {
    id: 2,
    title: "EdChang",
    category: "Branding",
    tags: [ "Identidad de marca", "Branding"],
    imageUrl: "edchang.png",
    link: "#",
    client: "EdChang",
    description: "Plataforma educativa revolucionaria que transforma la forma de aprender y enseñar en entornos digitales.",
    svgFiles: ["EdChang.svg"],
    additionalImages: [],
    videos: []
  },
  {
    id: 3,
    title: "M12",
    category: "Branding",
    tags: ["Diseño gráfico", "Identidad", "Branding"],
    imageUrl: "m12.png",
    link: "#",
    client: "M12 Interactive",
    description: "Experiencia digital interactiva de vanguardia que redefine la forma de percibir el espacio y el tiempo.",
    svgFiles: ["M12_1 (1).svg", "M12_2 (1).svg"],
    additionalImages: [],
    videos: []
  },
  {
    id: 4,
    title: "Veracruz",
    category: "Diseño gráfico",
    tags: ["Packaging", "Identidad", "Branding"],
    imageUrl: "veracruz.png",
    link: "#",
    client: "FashionBrand",
    description: "Plataforma de comercio electrónico con funcionalidades avanzadas y diseño premium para marca de moda.",
    svgFiles: ["veracruz_ (1).svg"],
    additionalImages: [],
    videos: []
  },
  {
    id: 5,
    title: "Florette",
    category: "Digital",
    tags: ["Creatividad", "RRSS", "Dirección de arte"],
    imageUrl: "florette.png",
    link: "#",
    client: "DataViz",
    description: "Dashboard para visualización de datos empresariales con gráficos interactivos y reportes personalizables.",
    svgFiles: ["Florette1.svg", "Florette2.svg", "Florette3.svg"],
    additionalImages: ["xtreme_grafica2.gif"],
    videos: []
  },
  {
    id: 6,
    title: "Astara",
    category: "Digital",
    tags: ["Creatividad", "RRSS", "Diseño gráfico"],
    imageUrl: "Ilustración_sin_título 12.png",
    link: "#",
    client: "ConsultPro",
    description: "Sitio web corporativo de alto impacto con sistema de gestión de contenidos personalizado.",
    svgFiles: [],
    additionalImages: ["ASTARA (1).gif", "ASTARA (2).gif", "ASTARA (1).jpg", "ASTARA (2).jpg", "ASTARA (3).jpg", "ASTARA (4).jpg","ASTARA (5).jpg","ASTARA (6).jpg"],
    videos: []
  },
  {
    id: 7,
    title: "Verleal",
    category: "Digital",
    tags: ["Creatividad", "RRSS", "Dirección de arte"],
    imageUrl: "verleal.png",
    link: "#",
    client: "ProductCatalog",
    description: "Catálogo digital interactivo con sistema de búsqueda avanzada y filtros personalizados.",
    svgFiles: [],
    additionalImages: ["verleal (6).jpg",  "verleal (5).jpg", "verleal (1).jpg", "verleal (1).gif", "verleal (2).jpg", "verleal (3).jpg", "verleal (4).jpg",
         "verleal (7).jpg", "verleal (8).jpg",
         "verleal (9).jpg", "verleal (10).jpg", "verleal (11).jpg", "verleal (12).jpg", "verleal (13).jpg",
         "verleal (14).jpg ", "verleal (15).jpg", "verleal (16).jpg"],
    videos: []
  },
  {
    id: 8,
    title: "Pickle",
    category: "Digital",
    tags: ["Diseño gráfico", "RRSS", "Branding"],
    imageUrl: "pickle.png",
    link: "#",
    client: "ProductCatalog",
    description: "Catálogo digital interactivo con sistema de búsqueda avanzada y filtros personalizados.",
    svgFiles: ["pickle_.svg"],
    additionalImages: [],
    videos: []
  },
  {
    id: 9,
    title: "Lidl",
    category: "Digital",
    tags: ["RRSS", "Creatividad", "Ilustración"],
    imageUrl: "lidl.png",
    link: "#",
    client: "LidL",
    description: "Landing page de alto rendimiento para el lanzamiento de un producto tecnológico con animaciones avanzadas.",
    svgFiles: [],
    additionalImages: [],
    videos: []
  },
  {
    id: 10,
    title: "Personal Stuff",
    category: "Digital",
    tags: ["Creatividad", "RRSS", "Dirección de arte"],
    imageUrl: "drew.png",
    link: "#",
    client: "ProductCatalog",
    description: "Catálogo digital interactivo con sistema de búsqueda avanzada y filtros personalizados.",
    svgFiles: [],
    additionalImages: ["BiutiBambu_1.jpg","BiutiBambu_2.jpg","YolySaa_1.jpg","YolySaa_2.jpg","YolySaa_3.jpg","YolySaa_4.jpg"],
    videos: []
  },
  {
    id: 11,
    title: "Palabras Intraducibles",
    category: "Ilustración",
    tags: ["Creatividad", "Ilustración"],
    imageUrl: "palabrasintraducibles.png",
    link: "#",
    client: "Xtreme Sports",
    description: "Identidad visual dinámica para marca deportiva con enfoque en deportes extremos y experiencias de alta adrenalina.",
    svgFiles: [],
    additionalImages: [],
    videos: []
  },
  {
    id: 12,
    title: "Merchan",
    category: "Digital",
    tags: ["Creatividad", "RRSS", "Dirección de arte"],
    imageUrl: "merchan.png",
    link: "#",
    client: "ProductCatalog",
    description: "Catálogo digital interactivo con sistema de búsqueda avanzada y filtros personalizados.",
    svgFiles: [],
    additionalImages: ["verleal (6).jpg",  "verleal (5).jpg", "verleal (1).jpg", "verleal (1).gif", "verleal (2).jpg", "verleal (3).jpg", "verleal (4).jpg",
         "verleal (7).jpg", "verleal (8).jpg",
         "verleal (9).jpg", "verleal (10).jpg", "verleal (11).jpg", "verleal (12).jpg", "verleal (13).jpg",
         "verleal (14).jpg ", "verleal (15).jpg", "verleal (16).jpg"],
    videos: []
  },
  
]; 

// Definición de tipos para los items del portfolio
export interface PortfolioItem {
  id: number;
  title: string;
  category: string;
  tags: string[];
  imageUrl: string;
  link: string;
  client: string;
  description: string;
  svgFiles?: string[];
  additionalImages?: string[];
  videos?: string[];
}