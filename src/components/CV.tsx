...
const CV = () => {
  ...
  const handleDownloadCV = async () => {
    try {
      ...
      await generatePDFFromElement('cv-content', 'CV_Andrea_Sanguino.pdf');
    } catch (error) {
  ...
  return (
    <section id="cv" className="section py-24">
      <div className="content-container">
        <div id="cv-content" className="pdf-container">
        ...
        <div className="text-center mt-8">
          ...
          >
            {isGeneratingPDF ? (
              <>
                <span className="animate-spin mr-2">⏳</span> Generando PDF...
              </>
            ) : (
              <>
                <Download size={18} className="mr-2" /> Generar y Descargar CV
              </>
            )}
          </button>
        </div>
